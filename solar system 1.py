...
class Window():
    def __init__(self, win):
        ...
        # // label for title
        Label(self.win, text="Welcome to the solar system simulation", font=("Roboto", 15), bg=background,
              fg=fontcolor).grid(row=0, column=0, columnspan=3)
        ...
        Label(self.win, text="x-position (starting point of the planet from x-axis)", bg=background, fg=fontcolor).grid(
            row=2, column=0)
        self.xpos_entry = Entry(self.win, width=5)
        self.xpos_entry.grid(row=2, column=1)

        # // label for y-position
        Label(self.win, text="y-position (starting point of the planet from y-axis)", bg=background, fg=fontcolor).grid(
            row=3, column=0)
        self.ypos_entry = Entry(self.win, width=5)
        self.ypos_entry.grid(row=3, column=1)

        # // label for z-position
        Label(self.win, text="z-position (starting point of the planet from z-axis)", bg=background, fg=fontcolor).grid(
            row=4, column=0)
        self.zpos_entry = Entry(self.win, width=5)
        self.zpos_entry.grid(row=4, column=1)

        # // label for x-velocity
        Label(self.win, text="x-velocity (the velocity the planet moving on x-axis)", bg=background, fg=fontcolor).grid(
            row=5, column=0)
        self.xvel_entry = Entry(self.win, width=5)
        self.xvel_entry.grid(row=5, column=1)

        # // label for y-velocity
        Label(self.win, text="y-velocity (the velocity the planet moving on y-axis)", bg=background, fg=fontcolor).grid(
            row=6, column=0)
        self.yvel_entry = Entry(self.win, width=5)
        self.yvel_entry.grid(row=6, column=1)

        # // label for z-velocity
        Label(self.win, text="z-velocity (the velocity the planet moving on z-axis)", bg=background, fg=fontcolor).grid(
            row=7, column=0)
    ...
    def update_values(self, event=None):
        ...
        planets.append(Planet(solarsys, mass=self.mass, position=(self.x_pos, self.y_pos, self.z_pos),
